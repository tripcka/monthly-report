...
export async function POST(request) {
  try {
    const { sheetUrl, hotelName, month } = await request.json();
    if (!sheetUrl || !sheetUrl.trim()) {
      return NextResponse.json({ error: "구글 시트 URL을 입력해 주세요." }, { status: 400 });
    }
    const rows = await loadGoogleSheetRows(sheetUrl.trim());
    const { kpis, tables, _note } = parseBlogExperience(rows, { hotelName, month });
    return NextResponse.json({ kpis, tables, note: _note });
  } catch (error) {
