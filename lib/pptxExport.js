...
import { LOGO_WHITE, SYMBOL_WHITE } from "./brandAssets";
...
function layoutImageSlot(ctx, s, y, label, srcs, showHeading = true) {
  ...
  while (remaining.length > 0) {
    const drawHeading = showHeading || page > 1;
    ...
    if (drawHeading) {
      sectionTitle(ctx.pres, curSlide, 0.6, curY, 12.1, label);
      curY += LABEL_H;
    }
...
export async function exportPptx({ hotelName, month, channels, channelData, outputType }) {
  ...
  // 1. Cover
  {
    ...
    s.addText(active.map((c) => c.title).join("  ·  "), { x: 0.7, y: 5.1, w: 8.5, h: 0.4, fontSize: 12.5, color: MUTED, fontFace: FONT, margin: 0 });
    // 장식 심볼 (오렌지 카드 위에 흰색 심볼)
    const boxW = 3.3, boxH = 2.6, boxX = 9.5, boxY = 4.5;
    s.addShape(pres.ShapeType.roundRect, { x: boxX, y: boxY, w: boxW, h: boxH, rectRadius: 0.18, fill: { color: ORANGE } });
    const symW = 1.8, symH = 1.8;
    s.addImage({ data: SYMBOL_WHITE, x: boxX + (boxW - symW) / 2, y: boxY + (boxH - symH) / 2, w: symW, h: symH });
  }
