...
function computeTableDisplayHeight(t, rows, w) {
  if (t.layout === "split" && rows.length > 1) {
    ...
    const halfW = (w - 0.3) / 2;
    const hLeft = computeTableHeight(left, halfW);
    const hRight = right ? computeTableHeight(right, halfW) : 0;
    return Math.max(hLeft, hRight);
  }
  return computeTableHeight(rows, w);
}
...
export async function exportPptx({ hotelName, month, channels, channelData, outputType }) {
  ...
  // 1. Cover
  {
    ...
    s.addText("월별 마케팅 운영 보고서", { x: 0.7, y: 3.6, w: 10, h: 0.9, fontSize: 50, color: "CBD5E1", fontFace: FONT, margin: 0 });
    s.addText(month || "[YYYY년 M월]", { x: 0.7, y: 4.7, w: 10, h: 0.5, fontSize: 15, color: ORANGE, bold: true, fontFace: FONT, margin: 0 });
  }
  ...
  function renderGroupContent(ch, group, data, s, y) {
    ...
    for (const t of tablesInGroup) {
      ...
      const estimatedHeight = computeTableDisplayHeight(t, rows, 12.1);
