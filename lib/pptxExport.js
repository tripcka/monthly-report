...
function layoutImageSlot(ctx, s, y, label, srcs) {
  ...
  while (remaining.length > 0) {
    if (curY > 6.6) {
      // 현재 슬라이드에 더 넣을 자리가 없으면 같은 섹션 제목으로 새 슬라이드 생성
    ...
    curSlide.addText(page === 1 ? label : `${label} (이어서)`, {
      x: 0.6, y: curY, w: 12.1, h: 0.28, fontSize: 12, color: NAVY, bold: true, fontFace: FONT, margin: 0,
    });
    curY += 0.32;

    const availRows = Math.max(1, Math.floor((6.9 - curY + IMG_GAP) / (IMG_CELL_H + IMG_GAP)));
