...
function computeTableDisplayHeight(t, rows, w) {
  ...
  if (t.layout === "flankedSummary") {
    const totalRowIndex = rows.findIndex((r) => r[0] === "__TOTAL__");
    const displayRows = totalRowIndex >= 0 ? rows.slice(0, totalRowIndex) : rows;
    const leftW = t.flankLeft ? 0.9 : 0;
    const rightW = totalRowIndex >= 0 ? 1.3 : 0;
    return computeTableHeight(displayRows, w - leftW - rightW, fontSize);
  }
...
function csvTable(pres, s, rows, x, y, w, fontSize = 9) {
  ...
  const tblRows = rows.map((r, ri) => r.map((cell) => ({
    ...
    options: {
      bold: ri === 0 || (groupedAdHeader && ri < 3), color: ri === 0 || (groupedAdHeader && ri < 3) ? WHITE : GRAYTXT,
      fill: ri === 0 || (groupedAdHeader && ri < 3) ? { color: NAVY } : (ri % 2 === 0 ? { color: "FAF8F5" } : { color: WHITE }),
...
export async function exportPptx({ hotelName, month, channels, channelData, outputType, hiddenSlides }) {
  const pptxgen = (await import("pptxgenjs")).default;
  const pres = new pptxgen();
  pres.layout = "LAYOUT_WIDE";
  const isHidden = (id) => hiddenSlides && hiddenSlides.has && hiddenSlides.has(id);
...
/** "주간/월간 유입" 표 — 왼쪽 주차별 표 + 오른쪽 "총유입" 병합 박스. rows 마지막 행은 ["__TOTAL__", 합계]. */
function weeklyInflowTable(pres, s, rows, x, y, w, fontSize = 9) {
  ...
  const totalBoxW = total !== null ? 1.7 : 0;
  const leftW = w - totalBoxW;
  const header = [["기간", "방문 횟수"]];
  const tableRows = [...header, ...weekRows];
  const rowHeights = computeTableRowHeights(tableRows, leftW, fontSize);
  const tblRows = tableRows.map((r, ri) => r.map((cell) => ({
    text: String(cell ?? ""),
    options: {
      bold: ri === 0, color: ri === 0 ? NAVY : GRAYTXT,
      fill: ri === 0 ? { color: "F0EDE7" } : (ri % 2 === 0 ? { color: "FAF8F5" } : { color: WHITE }),
      fontFace: FONT, fontSize, align: "center", valign: "middle",
    },
  })));
  s.addTable(tblRows, {
    x, y, w: leftW, colW: [leftW / 2, leftW / 2],
    rowH: rowHeights,
    border: { type: "solid", color: LIGHTGRAY, pt: 0.5 }, autoPage: false, fontFace: FONT,
  });
  ...
  if (total !== null) {
    s.addShape(pres.ShapeType.rect, {
      x: x + leftW, y, w: totalBoxW, h: totalHeight,
      fill: { color: "F0EDE7" }, line: { color: LIGHTGRAY, width: 0.5 },
    });
    s.addText("총유입", {
      x: x + leftW, y: y + totalHeight / 2 - 0.35, w: totalBoxW, h: 0.3,
      fontSize: fontSize + 1, bold: true, color: NAVY, fontFace: FONT, align: "center", margin: 0,
    });
    s.addText(`${total}회`, {
      x: x + leftW, y: y + totalHeight / 2 - 0.02, w: totalBoxW, h: 0.4,
      fontSize: fontSize + 6, bold: true, color: ORANGE, fontFace: FONT, align: "center", margin: 0,
    });
  }
  return totalHeight;
}

// "광고비 결제 인보이스"/"카드 결제 내역서" 표 — 왼쪽에 세로 병합 라벨칸, 오른쪽에
// 합계를 세로 병합해서 보여주는 칸. rows 마지막 행은 ["__TOTAL__", 합계]로 들어온다.
function flankedSummaryTable(pres, s, rows, x, y, w, flankLeft, flankRight, fontSize = 9) {
  if (!rows || rows.length < 2) return 0;
  const totalRowIndex = rows.findIndex((r) => r[0] === "__TOTAL__");
  const total = totalRowIndex >= 0 ? rows[totalRowIndex][1] : null;
  const header = rows[0];
  ...
  const leftW = flankLeft ? 0.9 : 0;
  const rightW = total !== null ? 1.3 : 0;
  const midW = w - leftW - rightW;
  const nCols = header.length;
  const tableRows = [header, ...dataRows];
  const rowHeights = computeTableRowHeights(tableRows, midW, fontSize);
  const totalHeight = rowHeights.reduce((sum, h) => sum + h, 0);

  if (leftW > 0) {
    s.addShape(pres.ShapeType.rect, { x, y, w: leftW, h: totalHeight, fill: { color: NAVY } });
    s.addText(flankLeft, {
      x, y, w: leftW, h: totalHeight, fontSize: fontSize + 1, bold: true, color: WHITE,
  ...
  s.addTable(tblRows, {
    x: x + leftW, y, w: midW, colW: new Array(nCols).fill(midW / nCols),
    rowH: rowHeights,
    border: { type: "solid", color: LIGHTGRAY, pt: 0.5 }, autoPage: false, fontFace: FONT,
  });
  ...
  if (total !== null) {
    s.addShape(pres.ShapeType.rect, {
      x: x + leftW + midW, y, w: rightW, h: totalHeight,
      fill: { color: "F0EDE7" }, line: { color: LIGHTGRAY, width: 0.5 },
    });
    s.addText(flankRight || "합계", {
      x: x + leftW + midW, y: y + totalHeight / 2 - 0.32, w: rightW, h: 0.28,
      fontSize: fontSize + 1, bold: true, color: NAVY, fontFace: FONT, align: "center", margin: 0,
    });
    s.addText(String(total), {
      x: x + leftW + midW, y: y + totalHeight / 2 - 0.02, w: rightW, h: 0.34,
      fontSize: fontSize + 3, bold: true, color: ORANGE, fontFace: FONT, align: "center", margin: 0,
    });
  }
  return totalHeight;
}

// 영수증/카드전표 카드 한 장을 간단한 사각 박스 + 텍스트 줄로 그린다.
// (픽셀 그대로 재현한 웹 미리보기와 달리, 편집 가능한 좌표 기반 PPTX에서는
// 필요한 정보를 모두 담은 컴팩트한 표 형태로 대체한다.)
function receiptCard(pres, s, x, y, w, h, title, lines) {
  s.addShape(pres.ShapeType.roundRect, { x, y, w, h, rectRadius: 0.05, fill: { color: WHITE }, line: { color: LIGHTGRAY, width: 0.75 } });
  s.addText(title, { x: x + 0.15, y: y + 0.1, w: w - 0.3, h: 0.24, fontSize: 10, bold: true, color: NAVY, fontFace: FONT, margin: 0 });
  s.addShape(pres.ShapeType.rect, { x: x + 0.15, y: y + 0.36, w: w - 0.3, h: 0.01, fill: { color: LIGHTGRAY } });
  const body = lines.map((l) => `${l.label}: ${l.value ?? "-"}`).join("\n");
  s.addText(body, {
    x: x + 0.15, y: y + 0.44, w: w - 0.3, h: h - 0.56, fontSize: 8, color: GRAYTXT, fontFace: FONT,
    margin: 0, valign: "top", lineSpacingMultiple: 1.25,
  });
}

// invoiceReceipts/cardSlipReceipts 배열(파싱된 객체들, 표 행렬이 아님)을 2개씩
// 나란히 카드로 그린다. 반환값은 사용한 높이(in).
function renderReceiptCards(pres, s, x, y, w, kind, items) {
  ...
  items.forEach((item, i) => {
    ...
    if (kind === "invoice") {
      receiptCard(pres, s, cx, cy, cardW, cardH, item.accountId ? `${item.accountId}의 영수증` : "Meta 광고비 영수증", [
        { label: "인보이스/결제 날짜", value: item.date },
        { label: "결제 수단", value: item.cardLast4 ? `Visa ····${item.cardLast4}` : null },
        { label: "참조 번호", value: item.refNo },
        { label: "소계", value: item.subtotal !== null ? `${Number(item.subtotal).toLocaleString("ko-KR")} KRW` : null },
        { label: "VAT", value: item.vat !== null ? `${Number(item.vat).toLocaleString("ko-KR")}원 (세율 ${item.vatRate || 10}%)` : null },
        { label: "최종 금액", value: item.total !== null ? `${Number(item.total).toLocaleString("ko-KR")}원` : null },
        { label: "제품 유형", value: item.productType },
      ]);
    } else {
      receiptCard(pres, s, cx, cy, cardW, cardH, "카드매출전표 (인터넷 재발급용)", [
        { label: "카드번호", value: item.cardLast4 ? `**** - **** - ${item.cardLast4}` : null },
        { label: "거래일시", value: item.date },
        { label: "매장명", value: item.merchantName },
        { label: "승인번호", value: item.approvalNo },
        { label: "가맹점번호", value: item.merchantNo },
        { label: "가맹점주소", value: item.merchantAddress },
        { label: "금액(AMOUNT)", value: item.amount !== null ? Number(item.amount).toLocaleString("ko-KR") : null },
        { label: "합계(TOTAL)", value: item.total !== null ? Number(item.total).toLocaleString("ko-KR") : null },
      ]);
    }
    used = Math.max(used, row * (cardH + 0.2) + cardH);
  });
  return used;
}
...
function renderTable(s, t, rows, x, y, w) {
    const fontSize = t.fontSize || 9;
    if (t.layout === "weeklyInflow") return weeklyInflowTable(pres, s, rows, x, y, w, fontSize);
    if (t.layout === "flankedSummary") return flankedSummaryTable(pres, s, rows, x, y, w, t.flankLeft, t.flankRight, fontSize);
    if (t.layout === "split" && rows.length > 1) {
      const header = rows[0];
      const dataRows = rows.slice(1);
      const splitAt = t.splitAt || Math.ceil(dataRows.length / 2);
  ...
  function renderGroupContent(ch, group, data, s, y) {
    ...
    if (group.layout === "sideBySide" && tablesInGroup.length > 1) {
      const renderable = tablesInGroup
        .map((t) => {
      ...
      if (renderable.length > 0) {
        const gap = 0.3;
        const halfW = (12.5 - gap * (renderable.length - 1)) / renderable.length;
        let maxUsed = 0;
        renderable.forEach(({ t, rows }, i) => {
          const usedHeight = renderTable(curSlide, t, rows, 0.42 + i * (halfW + gap), curY, halfW);
          maxUsed = Math.max(maxUsed, usedHeight);
        });
        curY += maxUsed + 0.25;
      }
    } else {
    for (const t of tablesInGroup) {
      ...
      // invoiceReceipts/cardSlipReceipts는 표(행렬)가 아니라 파싱된 인보이스/카드전표
      // 객체 배열이라서 일반 표 렌더링(renderTable/페이지네이션) 대신 전용 카드로 그린다.
      if (t.key === "invoiceReceipts" || t.key === "cardSlipReceipts") {
        const kind = t.key === "invoiceReceipts" ? "invoice" : "card";
        const used = renderReceiptCards(pres, curSlide, 0.42, curY, 12.5, kind, rows);
        curY += used + 0.25;
        continue;
      }

      if (t.layout !== "split" && t.layout !== "weeklyInflow" && t.layout !== "flankedSummary" && rows.length > 1) {
  ...
  for (const ch of active) {
    ...
    if (ch.slideGroups) {
      ...
      if ((overviewKpis.length > 0 || mergeGroupsWithData.length > 0 || overviewImages.length > 0) && !isHidden(`${ch.id}:overview`)) {
      ...
      otherGroups.forEach((group, groupIndex) => {
        ...
        if (isHidden(`${ch.id}:${groupIndex}:${group.title}`)) return;
        let s = pres.addSlide(); bgLight(s);
        pageTitle(pres, s, ch.kicker, ch.title);
        let groupY = 1.4;
    ...
    if (isHidden(`${ch.id}:simple`)) continue;
    let s = pres.addSlide(); bgLight(s);
    pageTitle(pres, s, ch.kicker, ch.title);
  ...
  const fileName = `[${hotelName || "호텔명"}] ${month || "YYYY-MM"} 마케팅 보고서.pptx`;
  if (outputType === "blob") {
