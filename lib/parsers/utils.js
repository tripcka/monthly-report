...
export function findCol(header, keywords) {
  const stripped = header.map((h) => String(h).replace(/\s/g, ""));
  // 1순위: 정확히 일치하는 열 (예: "비용" 키워드가 "클릭당비용" 같은 다른 열에 잘못 걸리는 것 방지)
  for (const kw of keywords) {
    const idx = stripped.findIndex((h) => h === kw);
    if (idx !== -1) return idx;
  }
  // 2순위: 부분 포함 (정확히 일치하는 열이 없을 때만)
  for (const kw of keywords) {
    const idx = stripped.findIndex((h) => h.includes(kw));
    if (idx !== -1) return idx;
  }
  return -1;
}
