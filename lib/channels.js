...
export const CHANNELS = [
  {
    ...
    kpis: [
      ...
      { key: "views30d", label: "조회수 (최근 30일)" },
      { key: "reactions30d", label: "반응 (최근 30일)" },
      { key: "profileActivity30d", label: "프로필 활동 (최근 30일)" },
    ],
    tables: [
      { key: "posts", label: "게시물별 성과" },
      { key: "ads", label: "광고 진행 내역" },
    ],
    images: [{ key: "receipt", label: "영수증 이미지 첨부 공간" }],
    // 인스타그램은 계정 인사이트 페이지에서 브라우징으로 가져오는 방식이라 CSV 업로드 대상이 아님
    // (숫자는 브라우징 결과를 KPI 입력창에 직접 채운다). 게시물/광고 내역만 CSV 업로드 지원.
    uploads: [
      { key: "postsCsv", label: "게시물별 성과 CSV", parser: "generic", targetTable: "posts" },
      { key: "adsCsv", label: "광고 진행 내역 CSV", parser: "generic", targetTable: "ads" },
    ],
  },
  {
    id: "blogExperience",
  ...
  },
  {
    ...
    tables: [
      ...
      { key: "keywordsPc", label: "PC 클릭 키워드 TOP 20" },
      { key: "moSummary", label: "MO 당월 데이터" },
      { key: "keywordsMo", label: "MO 클릭 키워드 TOP 20" },
      { key: "hourlyPc", label: "시간대별 데이터 (PC)" },
      { key: "hourlyMo", label: "시간대별 데이터 (모바일)" },
    ],
...
export function isChannelActive(channelData, channel) {
  ...
  const hasImage = Object.values(channelData.images || {}).some((v) => !!v);
