...
export const CHANNELS = [
  {
    ...
    tables: [
      ...
      { key: "invoiceSummary", label: "광고비 결제 인보이스", layout: "flankedSummary", flankLeft: "광고비\n인보이스", flankRight: "합계" },
      { key: "invoiceReceipts", label: "광고비 결제 인보이스 (영수증)" },
      { key: "cardSummary", label: "광고비 카드 결제 내역", layout: "flankedSummary", flankLeft: "카드\n결제\n내역서", flankRight: "최종 청구액" },
      { key: "cardSlipReceipts", label: "광고비 카드 결제 내역 (전표)" },
    ],
    ...
    slideGroups: [
      ...
      // 인보이스/카드전표 원문을 붙여넣기로 자동 입력하는 호텔만 해당 (붙여넣은 데이터가
      // 없으면 groupHasData가 false를 반환해서 이 슬라이드 자체가 자동으로 생략된다).
      { title: "광고비 결제 인보이스", tableKeys: ["invoiceSummary", "invoiceReceipts"] },
      { title: "광고비 카드 결제 내역", tableKeys: ["cardSummary", "cardSlipReceipts"] },
    ],
    uploads: [
  ...
  },
  {
    ...
    kpis: [],
    tables: [
      { key: "rosterBlog", label: "체험단 진행 현황" },
      { key: "rosterInstagram", label: "인플루언서 진행 현황" },
    ],
    images: [{ key: "exposureCapture", label: "체험단 상위노출" }],
    // 블로그 체험단과 인스타그램 인플루언서는 실제 다른 채널이라 별개 슬라이드로 나눈다.
    // (포스팅 URL에 blog/naver가 있으면 블로그, instagram이 있으면 인스타그램으로 자동 구분됨 — lib/parsers/blogExperience.js 참고)
    slideGroups: [
      { title: "블로그체험단", tableKeys: ["rosterBlog"] },
      { title: "인스타그램 인플루언서", tableKeys: ["rosterInstagram"] },
      { title: "체험단 상위노출", imageKeys: ["exposureCapture"] },
    ],
    uploads: [
      { key: "rosterCsv", label: "체험단 리스트 CSV (성함/일방문자수·팔로워/방문일/포스팅URL/상위노출)", parser: "blogExperience" },
    ],
  },
  {
    id: "naverSearch",
