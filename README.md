...
- **미리보기 그대로 PPTX**: 오른쪽 미리보기의 각 1280×720 페이지를 고해상도 이미지로 캡처해
  16:9 슬라이드 전체에 넣습니다. 미리보기와 표·이미지·여백이 동일하게 나오지만, PowerPoint에서
  글자와 표를 개별 편집할 수는 없습니다.
- **편집 가능한 PPTX**: 기존 좌표 기반 출력입니다. PowerPoint에서 글자·표·이미지를 개별 편집할 수
  있지만 브라우저 미리보기와 간격이 일부 다를 수 있습니다.

Google Drive 저장에서도 위 두 방식 중 하나를 선택할 수 있습니다.
...
- 이 앱은 채널별로 **실제 검증된 파일 포맷을 자동으로 인식해서 계산**합니다:
  - 네이버 검색광고 키워드 리포트 CSV → PC/모바일 통합 집계 + TOP20 키워드 자동 정렬
